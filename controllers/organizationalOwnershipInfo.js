  ...
  updateOrganizationalOwnershipInfo: async (req, res, next) => {
    try {
      ...
      for (const item of updatesArray) {
        let {
          id,
          organizational_info_id, // used only for INSERT
          role,
          title,
          first_name,
          last_name,
        ...
        if (id) {
          ...
          // INSERT with generated UUID and organization_info_id
          id = uuidv4();

          if (!organizational_info_id) {
            console.warn("Skipping insert: organization_info_id is missing");
            continue;
          }

          columns.unshift("organizational_info_id");
          placeholders.unshift("?");
          values.unshift(organizational_info_id);
